...
def get_base_columns(df):
    ...
    for column in df.columns:
        if len(column) == 2 and column[0] in meta.utils.ACTG and column[1] in meta.utils.ACTG:
...
def compute_mismatches(cfg):
    ...
    df = (
        pd.read_csv(cfg.filename_mismatch, sep="\t", names=columns)
...
def load_df_mismatches(cfg):
    ...
    if parquet.exists(cfg.forced):
        ...
        if meta.utils.metadata_is_similar(metadata_file, metadata_cfg, include=include):
