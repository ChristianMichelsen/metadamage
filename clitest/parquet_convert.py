...
def save_parquet_dir_to_csv(dir_parquet, dir_csv):
    """Takes a single parquet file as input, loads it into pandas and saves as csv file in output directory"""
    df = pq.read_table(dir_parquet).to_pandas()
    file_csv = dir_csv / dir_parquet.with_suffix(".csv").name
...
def convert_fit_predictions(dir_parquet, dir_csv=None):
    # make sure that output folder exists

    if dir_csv is None:
        dir_csv = dir_parquet.parent / "csv" / dir_parquet.name

    dir_csv.mkdir(parents=True, exist_ok=True)

    save_parquet_dir_to_csv(dir_parquet, dir_csv)
