...
mismatch_parquet_foldername = "mismatch_parquet"
fit_results_foldername = "fit_results"

mismatch_suffix = ".mismatch.txt"
mismatch_stat_suffix = ".stat.txt"
...
def find_mismatch_files(storage_path):
    """Keeps only files that ends with .mismatch.
    Returns as Paths
    """
    return sorted((storage_path / mismatch_txt_foldername).glob(f"*{mismatch_suffix}"))
...
class Configs:
    def __init__(
        ...
        self.filenames = find_mismatch_files(self.storage_path)
    ...
    def __repr__(self):
        s = f"Configs(filenames = {self.filenames}, storage_path = {self.storage_path})"
        return s
...
def make_sure_parent_exists(path):
    if not path.parent:
...
class Config:
    ...
    def get_file_name(self, foldername, filename):
        ...
        make_sure_parent_exists(path)
        return path

    @property
    def filename_mismatches_parquet(self):
        return self.get_file_name(
            foldername=mismatch_parquet_foldername,
            filename=f"{self.shortname}.mismatch.parquet",
        )

    @property
    def filename_fit_results(self):
        return self.get_file_name(
            foldername=fit_results_foldername,
            filename=f"{self.shortname}.fit_results.parquet",
        )

    # @property
    # def filename_results(self):
    #     return self.storage_path / "results" / f"{self.shortname}.results.parquet"

    # @property
    # def filename_results_read(self):
    #     return self.storage_path / "reads" / f"{self.shortname}.results.read.parquet"
...
def is_df_mismatch_accepted(cfg, df_mismatch):
